...
func (m *Document) SetLinkBookDocuments(book_id int, link_docs string) (err error) {
...
func (m *Document) UpdateLinkBookDocument(doc_id int) (err error) {
	o := orm.NewOrm()
...
func (m *Document) UpdateLinkBookDocuments(book_id int) (err error) {
	o := orm.NewOrm()
	sql := `UPDATE md_documents a,md_documents b SET 
		a.document_name = b.document_name,
		a.identify = b.identify,
...
func (m *Document) GetLinkBookDocuments(book_id int) (doclinks string, doclist string, err error) {
	...
	doclist = ""
	if count > 0 {
		trees := make([]*DocumentLinkTree, count)
...
func (m *Document) FixedBookDocuments(book_id int) (err error) {
	o := orm.NewOrm()
...
//分页查询指定项目的链接
func (m *Document) FindToLinksPager(pageIndex, pageSize, booKId int) (books []*BookResult, totalCount int, err error) {
