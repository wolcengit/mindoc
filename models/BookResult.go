...
func (m *BookResult) Converter(sessionId string) (ConvertBookResult, error) {
	...
	docs, err := NewDocument().FindListByBookIdOrLink(m.BookId, m.LinkId, m.LinkDoc)
	if err != nil {
	...
	ebookConfig := converter.Config{
		...
		Footer:       "<p style='color:#8E8E8E;font-size:12px;'>Created By <a href='https://github.com/wolcengit/mindoc' style='text-decoration:none;color:#1abc9c;font-weight:bold;'>Z-MinDoc</a> <span style='float:right'>- _PAGENUM_ -</span></p>",
	...
	if m.Publisher != "" {
		ebookConfig.Footer = "<p style='color:#8E8E8E;font-size:12px;'>Created By  <span style='text-decoration:none;color:#1abc9c;font-weight:bold;'>" + m.Publisher + "</span>  <span style='float:right'>- _PAGENUM_ -</span></p>"
	}
	if m.RealName != "" {
...
func (m *BookResult) FindToPagerByLink(pageIndex, pageSize int, linked int) (books []*BookResult, totalCount int, err error) {
	o := orm.NewOrm()
	book := NewBook()
	...
	if linked == 1 {
		sql1 = "SELECT COUNT(book.book_id) AS total_count FROM " + book.TableNameWithPrefix() + " AS book WHERE book.link_id > 0 "
	} else {
		sql1 = "SELECT COUNT(book.book_id) AS total_count FROM " + book.TableNameWithPrefix() + " AS book WHERE book.link_id = 0 "
	}
	...
	_, err = o.Raw(sql2, offset, pageSize).QueryRows(&books)
